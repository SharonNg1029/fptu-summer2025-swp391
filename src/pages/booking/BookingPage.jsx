...
import { FaCalendarAlt, FaClock, FaUser, FaPhone, FaEnvelope, FaIdCard, FaUsers, FaMapMarkerAlt, FaCreditCard, FaQrcode, FaArrowLeft } from 'react-icons/fa';
...
const ConfirmBookingModal = ({ visible, onCancel, bookingData, onConfirm, paymentMethod: paymentMethodProp }) => {
  ...
  const getCostBreakdown = () => {
    ...
    const currentCollectionMethod = bookingData?.selectedCollectionMethod || bookingData?.collectionMethod;
  ...
  const handleVNPAYPayment = async () => {
    ...
    try {
      ...
      processingMsg = message.loading('Processing VNPAY payment, please wait a moment...', 0);
      ...
      if (data.vnpUrl) {
        ...
        processingMsg();      
        message.success('Payment creation successful! Redirecting to VNPAY...', 2);        
        setTimeout(() => {
...
const generatePDF = async (shouldDownload = true) => {
  ...
  try {
    ...
    if (serviceCost > 0) {
      costTableBody.push([
        ...
        { text: `${serviceCost.toLocaleString()} VND`, alignment: 'right', bold: true }
      ]);
    }
    ...
    if (medicationMethod === 'staff-collection') {
      costTableBody.push([
        { text: 'Phí nhân viên thu thập tại nhà', alignment: 'left', bold: true },
        { 
    ...
    if (isExpressService && expressCost > 0) {
      costTableBody.push([
        { text: 'Phí dịch vụ Express (Xử lý ưu tiên)', alignment: 'left', color: '#fa8c16', bold: true },
    ...
    costTableBody.push(
      ...
      [
        ...
        { text: `${calculatedTotal.toLocaleString()} VND`, bold: true, alignment: 'right', color: '#e91e63', fontSize: 14 }
      ]
    );
        ...
        },
        {
          columns: [
            ...
            },
            {
              width: 200,
              stack: [
                { text: `Ngày ${new Date().getDate()}/${new Date().getMonth() + 1}/${new Date().getFullYear()}`, alignment: "center", italics: true, margin: [0, 0, 0, 8] },
                { text: "Người yêu cầu", alignment: "center", italics: true, margin: [0, 0, 0, 3] },
                signatureImg
                  ? { image: signatureImg, width: 100, alignment: "center", margin: [0, 3, 0, 3] }
                  : { text: "(Chưa ký)", alignment: "center", margin: [0, 10, 0, 3] },
                {
           ...
           columns: [
             ...
             },
             {
               ...
               stack: [
                 ...
                 signatureImg
                   ? { image: signatureImg, width: 100, alignment: "center", margin: [0, 5, 0, 3] }
                   : { text: "(Chưa ký)", alignment: "center", margin: [0, 5, 0, 3] },
  ...
  const renderSummary = () => {
    const { serviceType, service, collectionMethod, medicationMethod, appointmentDate, timeSlot, isExpressService, totalCost, firstPerson, secondPerson, homeAddress, selectedKitType } = bookingData;
    const { serviceCost, collectionCost, mediationCost, expressCost } = getCostBreakdown();
      ...
      <div>
        ...
        >
          <Row gutter={[16, 16]}>
            ...
            <Col span={12}>
              <div style={{ padding: '12px 16px', backgroundColor: '#fafafa', borderRadius: 8, border: '1px solid #f0f0f0' }}>
                <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>TESTING KIT</Text>
                <Text strong style={{ fontSize: 14 }}>
        ...
        <Card 
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 18, fontWeight: 600, color: '#52c41a' }}>Test Subject Information</span>
            </div>
          }
          style={{ 
            marginBottom: 20, 
        ...
        <Card 
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 18, fontWeight: 600, color: '#fa8c16' }}>Cost Details</span>
            </div>
          }
          style={{ 
            marginBottom: 20, 
          ...
          <div style={{ padding: '8px 0' }}>
            ...
            {bookingData.collectionMethod && (
              <Row justify="space-between" style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: isExpressService ? '#f6ffed' : '#fafafa', borderRadius: 6, border: isExpressService ? '1px solid #b7eb8f' : 'none' }}>
                <Text>🔬 Collection Fee ({bookingData.collectionMethod?.name})</Text>
                {isExpressService ? (
                  <Text strong style={{ color: '#52c41a' }}>Free (0 VND)</Text>
                ) : (
            ...
            {medicationMethod === 'staff-collection' && (
              <Row justify="space-between" style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: '#f6ffed', borderRadius: 6, border: '1px solid #b7eb8f' }}>
                ...
                {isExpressService ? (
                  ...
                  <Text strong style={{ color: '#52c41a' }}>500,000 VND</Text>
                )}
              </Row>
            )}
            ...
            {medicationMethod === 'postal-delivery' && (
              <Row justify="space-between" style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: '#e6f7ff', borderRadius: 6, border: '1px solid #91d5ff' }}>
                ...
                {isExpressService ? (
                  <Text strong style={{ color: '#1890ff' }}>Free (0 VND)</Text>
                ) : (
                  <Text strong style={{ color: '#1890ff' }}>250,000 VND</Text>
                )}
              </Row>
            )}
            ...
            {medicationMethod === 'express' && (
              <Row justify="space-between" style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: '#fff7e6', borderRadius: 6, border: '1px solid #ffd591' }}>
                ...
                {isExpressService ? (
                  <Text strong style={{ color: '#fa8c16' }}>Free (0 VND)</Text>
                ) : (
                  <Text strong style={{ color: '#fa8c16' }}>700,000 VND</Text>
                )}
              </Row>
            )}
            ...
            {isExpressService && (
              <Row style={{ marginBottom: 12, padding: '8px 12px', backgroundColor: '#f6ffed', borderRadius: 6, border: '1px dashed #52c41a' }}>
                <Text style={{ color: '#389e0d', fontSize: '13px', fontStyle: 'italic', width: '100%', textAlign: 'center' }}>
                  💡 When using Express Service, all collection methods and transportation are free (0 VND)
                </Text>
              </Row>
            )}
        ...
        <Card 
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 18, fontWeight: 600, color: paymentMethod === 'cash' ? '#52c41a' : '#1890ff' }}>Payment Method</span>
            </div>
          }
          style={{ 
            borderRadius: 12, 
          ...
          <div style={{ 
            ...
            {paymentMethod === 'cash' ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                ...
                <Text strong style={{ fontSize: 16, color: '#52c41a' }}>💵 Cash payment on service receipt</Text>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                  <QrcodeOutlined style={{ fontSize: 24, color: '#1890ff' }} />
                  <Text strong style={{ fontSize: 16, color: '#1890ff' }}>📱 Payment via VNPAY</Text>
              </div>
            )}
          </div>
        </Card>
  ...
  const renderSignature = () => (
    <div style={{ 
      ...
      <div style={{ 
        ...
        💡 Draw your signature in the box above using your mouse or finger.<br/>
 ...
 const validateAge18 = (_, value) => {
  ...
  const validateSecondPersonAge = (_, value) => {
    ...
    if (relationship !== 'Child' && age < 18) {
      return Promise.reject(
        new Error(`People with relationships "${relationship}" must be 18 years or older!`)
      );
    }
  ...
  const validatePhoneNumber = (_, value) => {
    ...
    if (!phoneRegex.test(value)) {
      return Promise.reject(new Error('Phone number must be 10-11 digits (can start with +84)!'));
    }
    return Promise.resolve();
  };
    ...
    if (firstPersonRelationship && value && selectedService?.name) {
      if (!isValidRelationshipPair(selectedService.name, firstPersonRelationship, value)) {
        if (selectedService.name === 'DNA Testing for Birth Certificate') {
          ...
          return Promise.reject(new Error(`With relationship"${firstPersonRelationship}" of the first person, the second person must choose one of the relationships: ${validOptions.join(', ')}`));
        } else {
          ...
          return Promise.reject(new Error(`Invalid relationship pair for this service. Valid pairs:${pairNames}`));
        }
      }
    }
  ...
  navigate('/'); // ✅ Đúng - dùng function
    ...
    let errorMessage = 'An error occurred while saving the booking information!';
    if (error.response) {
                ...
                <div className="mt-4 p-4 border border-blue-200 rounded-lg bg-blue-50">
                  <div className="flex items-start mb-3">
                    ...
                    <div className="flex-1">
                      <label className="block text-sm font-medium text-blue-700 mb-2">Home address*</label>
                  ...
                  {areAllTimeSlotsDisabled() && (
                    <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <div className="flex items-center">
                        ...
                        All slots for today have been filled. Please come back another day!
                        </span>
                      </div>
                    </div>
                  )}
              ...
              {appointmentDate && selectedMedicationMethod === 'postal-delivery' && (
                <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center">
                    ...
                    With postal delivery, you just need to choose a date. The kit will be delivered to your address on the selected date.
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              {isExpressService && (
                <div className="text-sm text-green-600 bg-green-50 p-2 rounded-md mt-2 italic text-center">
                  💡 When using Express Service, all sample collection and shipping methods are Free
                </div>
              )}
